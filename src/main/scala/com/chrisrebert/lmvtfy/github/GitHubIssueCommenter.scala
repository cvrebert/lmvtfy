  ...
  override def receive = {
    case ValidationResult(messagesMarkdown, mention@LiveExampleMention(example: LiveExample, GitHubUser(username), GitHubIssue(repo, issue))) => {
      ...
      val commentMarkdown = s"""
        |Hi @${username}!
        |
        |You appear to have posted a live example (${exampleUrl}), which is always a good first step. However, according to [the HTML5 validator](http://html5.validator.nu), **your example has some validation errors**, which might potentially be causing your issue:
        |${messagesMarkdown}
        |
